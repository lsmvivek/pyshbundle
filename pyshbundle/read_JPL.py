...
def read_JPL(file_path):
    ...
    coeff_pattern = re.compile(r'^GRCOF2\s+(\d+)\s+(\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)')
    # coeff_pattern = re.compile(r'^GRCOF2\s+(\d+)\s+(\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+([-+]?\d*\.\d+e[-+]?\d+)\s+([-+]?\d*\.\d+e[-+]?\d+).*')
    ...
    # Example of how to access the coefficients and dates
    # print("Time Coverage Start:", data['time_coverage_start'])
    # print("Time Coverage End:", data['time_coverage_end'])

    # degree = 2
    # order = 0
    # if (degree, order) in data['coefficients']:
    #     print(f"Clm for degree {degree} and order {order}: {data['coefficients'][(degree, order)]['Clm']}")
    #     print(f"Slm for degree {degree} and order {order}: {data['coefficients'][(degree, order)]['Slm']}")
    # else:
    #     print(f"No data for degree {degree} and order {order}")

    # # Print all coefficients (Optional)
    # # for key, value in data['coefficients'].items():
    # #     print(f"Degree: {key[0]}, Order: {key[1]}, Clm: {value['Clm']}, Slm: {value['Slm']}")